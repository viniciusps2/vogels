...
Query.prototype.filterExpression = function(expression) {
  ...
  this.request.FilterExpression = (this.request.ExpressionAttributeNames||'') + 
      (this.request.FilterExpression?' AND':'')+ expression;

  return this;
};

Query.prototype.keyConditionExpression = function(expression) {
  
  this.request.KeyConditionExpression = (this.request.KeyConditionExpression||'') + 
      (this.request.KeyConditionExpression?' AND':'')+ expression;
...
Query.prototype.expressionAttributeValues = function(data) {
  ...
  this.request.ExpressionAttributeValues = _.merge(this.request.ExpressionAttributeValues||{},data);

  return this;
};

Query.prototype.expressionAttributeNames = function(data) {
  this.request.ExpressionAttributeNames = _.merge(this.request.ExpressionAttributeNames||{},data);;
...
Query.prototype.lastEvaluatedKey = function (keys) {
  this.request.ExclusiveStartKey = keys;
...
console.log('+++++++++++++++++++++')
  ...
  this.keyConditionExpression('#'+key+' = :'+key+'_value');

  var names={};names['#'+key]=key;
  this.expressionAttributeNames( names );

  var values={};values[':'+key+'_value']=this.hashKey;
